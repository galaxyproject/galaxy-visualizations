...
describe("terminal states", () => {
  ...
  it("knows which job states will not change again", () => {
    for (const s of ["ok", "error", "deleted", "discarded"])
      expect(isTerminal("job", s)).toBe(true);
    for (const s of ["new", "queued", "running", "paused"])
      expect(isTerminal("job", s)).toBe(false);
  });
...
describe("galaxyStateReader", () => {
  ...
  it("reports a job's state as Galaxy gives it", async () => {
    ...
    expect(
      await galaxyStateReader("/", "include")({ kind: "job", id: "j1", label: "run_tool" }),
    ).toBe("running");
  });
