...
const DISCARDED = 'Tool call "get_tool_details" returned 91 KB, over the 64 KB limit for a single '
    + "result, so it was discarded.";

function panel() {
    return { addUserMessage: vi.fn(), addToolCard: vi.fn(), updateToolCard: vi.fn(),
             startAssistantMessage: vi.fn(), appendDelta: vi.fn(), finishAssistantMessage: vi.fn() };
}

describe("a restored session renders a failed step as failed", () => {
    const stored = [{ role: "tool", tool_call_id: "c1", name: "get_tool_details", content: DISCARDED }];
