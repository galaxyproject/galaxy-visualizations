...
async def _eval_top_level_await(code, namespace):
    ...
    pending = eval(compile(parsed, "<olit>", "exec", flags=TOP_LEVEL_AWAIT), namespace)
        ...
        try:
            with contextlib.redirect_stdout(buffer):
                if eval_code_async is not None:
                    result = await eval_code_async(code, globals=self._ns, filename="<olit>")
                else:
...
def _failure_text(exc):
    ...
    frames = [f for f in traceback.extract_tb(exc.__traceback__) if f.filename == "<olit>"]
    lines = []
    if frames:
