...
`LLM_PROVIDER` names an entry in `brain/olite/substrate/llm/providers.py`, which carries the
...
`npm run seams` compares OLite against an Orbit checkout at `LOOM_ROOT` (`seams/README.md`).
