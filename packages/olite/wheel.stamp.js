...
const STAMPED = /^olite-.*-0[0-9a-f]{12}-py3-none-any\.whl$/;

function main() {
    const wheels = readdirSync(DIST).filter((f) => f.startsWith("olite-") && f.endsWith(".whl"));
    if (wheels.length !== 1) {
    ...
    // olite-<version>-<tag>-py3-none-any.whl
    const stamped = wheel.replace(/^(olite-[^-]+)-/, `$1-${tag}-`);
